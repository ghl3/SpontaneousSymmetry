...
$$
\text{mean} = \tilde{\Sigma} (\Sigma + \tilde{\Sigma})^{-1} x + \Sigma  (\Sigma + \tilde{\Sigma})^{-1} \tilde{x}
$$

$$
\text{covariance} = \Sigma (\Sigma + \tilde{\Sigma})^{-1} \tilde{\Sigma}
$$
...
\mu_{i+1} & \leftarrow & \tilde{\Sigma'}_{i+1} (\Sigma'_{i+1} + \tilde{\Sigma'}_{i+1})^{-1} \mu_{i+1}' + \Sigma'_{i+1} (\Sigma'_{i+1} + \tilde{\Sigma'}_{i+1})^{-1} \tilde{x}_{i+1} \\
\Sigma_{i+1} & \leftarrow & \Sigma'_{i+1} (\Sigma'_{i+1} + \tilde{\Sigma'}_{i+1})^{-1}\tilde{\Sigma'}_{i+1}\\
\end{array}
$$
...
In the previous example, demonstrated a simple example of a Kalman Filter by building a gaussian model of a state vector, updating it with fixed dynamics between time steps, and incorporating measurements with gaussian errors.

We shall now walk through a more complicated version of this model where we increase the complexity by allowing the dynamics of the system to vary at each time step and to be non-determinisic.
...
To include time-varying influence on the system, we will add a term that can vary at each time step, known as the "influence" term.  We will denote it as $\overleftrightarrow{B}\vec{u_i}$ (in principal, we could just include this as a single vector at each time $t_i$ and not include the $B$ matrix, but we add it here to adhere to conventions and also to convey that the source of the influence is different than its effect on the state vector).
...
Adding noise to the dynamics makes this update step more complicated.  For a known state vector, one can think of the noise in the following way: First, make a deterministic dynamic update to the state vector, as above.  Then, randomly draw from a distribution agiveround that newly updated vector using a gaussian distribution, which then becomes our updated state vector.

If, instead of a fixed vector, our state at time $t_i$ is described by a distribution of vectors $p_i(\vec{x_i})$, we must determine how to update this distribution by adding noise.  This can be done by performing a convolution between the initial state distribution the noise distribution.  From the laws of probability, we know:
...
$$
p(\vec{x}_{noise}) = \int p(\vec{x}_{noise} | \vec{x}) * p(\vec{x}) d\vec{x}
$$

Where the first term in the integral, $p(\vec{x}_{noise} | \vec{x})$, is the noise distribution and the second term, $p(\vec{x})$, is the distribution of the state before adding noise (but after doing the deterministic part of the dynamic update).  We will assume that this added noise is a gaussian with mean of 0 and a known covariance matrix  $\overleftrightarrow{Q}$ (it is assumed that we know this in advance).   The state vector distribution and the noise are therefore both gaussians, and because the convolution of two gaussians is itself a gaussian, we can write the update step due to noise as:
